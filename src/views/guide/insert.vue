<template>
  ...
  <div class="form-group mb-2">
    <button @click="submit" class="btn btn-alt-primary">ثبت</button>
  </div>
</template>
<script>
  ...
  methods: {
    submit(){
      if(this.title === '' || this.body === '' || this.url === ''){
        ...
        //submit data
        let data = axios.post('/api/guide/insert',{'title':this.title,'cat':this.cat,'body':this.body,'url':this.url})
            .then((response)=>{
