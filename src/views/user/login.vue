<template>
  <form
    id="loginMainSite"
    method="post"
    :action="this.$filters.getApiUrl() + '/login/by/token/profile/business'"
  >
  ...
  <main id="main-container">
    ...
    <div
      class="bg-image"
      style="background-image: url('/assets/media/photos/photo21.jpg')"
    >
      <div class="row g-0 justify-content-center bg-primary-dark-op">
        <!-- pwa install banner -->
        <div
          v-if="installBanner"
          class="row bg-body-extra-light position-absolute rounded p-2 mt-2"
          style="width: 350px"
        >
          <div class="col-8">
        ...
        <div
          class="hero-static col-sm-8 col-md-6 col-xl-4 d-flex align-items-center p-2 px-sm-0"
        >
          <!-- Sign In Block -->
          <div
            class="block block-transparent block-rounded w-100 mb-0 overflow-hidden"
          >
            <div
            ...
            >
              ...
              <form v-show="!this.loading" @submit.prevent="submit">
                ...
                <div
                  class="d-sm-flex justify-content-sm-between align-items-sm-center text-center text-sm-start"
                >
                  <div class="fw-semibold fs-sm py-0">
                    <RouterLink to="/user/forget-password"
                      >کلمه عبور را فراموش کرده اید؟</RouterLink
                    >
                  </div>
                </div>
    ...
    callbtn() {
      if (this.installPromptEvent) {
        ...
        Swal.fire({
          ...
          text: "در قسمت برنامه های تلفن همراه، میتوانید آوَپ را پیدا کنید",
          icon: "warning",
      ...
      if (
        ...
        axios
          ...
          .then((response) => {
            ...
            document
              .getElementById("loginTokenID")
              .setAttribute("value", response.data.tokenID);
    ...
    axios
      ...
      .then((response) => {
        if (response.data.result === true) {
          document
            .getElementById("loginTokenID")
            .setAttribute("value", localStorage.getItem("X-AUTH-TOKEN"));
          document.getElementById("loginMainSite").submit();
        } else {
          this.loading = false;
        }
