    ...
    <v-row>
      ...
      <v-col cols="12" sm="12" md="10">
        <v-text-field v-model="user_email" :label="$t('user.email')" prepend-inner-icon="mdi-email" :disabled="true"
          color="primary" variant="solo" :loading="loading"></v-text-field>
        <v-text-field v-model="user_mobile" :label="$t('user.mobile')" prepend-inner-icon="mdi-cellphone"
          :disabled="true" color="primary" variant="solo" :loading="loading"></v-text-field>
        <v-text-field v-model="user_fullname" :label="$t('user.name')" prepend-inner-icon="mdi-account"
          :disabled="loading" :loading="loading" color="primary" variant="solo"></v-text-field>
        <v-btn color="primary" :disabled="loading" :loading="loading" prepend-icon="mdi-content-save"
