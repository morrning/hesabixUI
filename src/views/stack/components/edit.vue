...
export default {
  ...
  async beforeMount() {
    ...
    //get content
    let content = await axios.get('/api/stack/content/get/' + this.$route.params.id);
    this.title = content.data.title;
    this.body = content.data.body;
    this.cat = content.data.cat;
  ...
  methods:{
    submit(){
      if(this.title === '' || this.body === ''){
        ...
        //submit data
        let data = axios.post('/api/stack/edit/' + this.id,{'title':this.title,'cat':this.cat,'body':this.body})
            .then((response)=>{
