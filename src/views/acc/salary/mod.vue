  ...
  methods: {
    loadData(id = '') {
      if (id != '') {
        //load user info
        this.isLoading = true;
        axios.post('/api/salary/info/' + id).then((response) => {
    ...
    save() {
      if (this.data.name.length === 0)
        ...
        this.isLoading = true;
        axios.post('/api/salary/mod/' + this.data.code, this.data).then((response) => {
          this.isLoading = false;
          if (response.data.result == 2) {
            ...
            Swal.fire({
              text: 'مشخصات تنخواه‌گردان ثبت شد.',
              icon: 'success',
              confirmButtonText: 'قبول'
            }).then(() => {
              this.$router.push('/acc/salary/list')
            });
          }
        })
      }
